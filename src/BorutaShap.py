...
class BorutaShap:
    ...
    def check_model(self):
        ...
        # Note: We can't check for feature_importances_ before fitting
        # Tree-based models only have this attribute after fitting
    ...
    def fit(self, X, y, sample_weight = None, n_trials = 20, random_state=0, sample=False,
        ...
        # Use tqdm only if verbose=True
        iterator = tqdm(range(self.n_trials)) if verbose else range(self.n_trials)
    ...
    def feature_importance(self, normalize):
        ...
        if self.importance_measure.lower() == 'shap':
        ...
        elif self.importance_measure.lower() == 'perm':
        ...
        elif self.importance_measure.lower() == 'gini':
        ...
        if self.sample:
            ...
            if self.classification:
                ...
                if isinstance(self.shap_values, list):
                    ...
                    self.shap_values = shap_imp / len(self.shap_values)

                elif isinstance(self.shap_values, np.ndarray) and len(self.shap_values.shape) == 3:
                    # 3D array: (samples, features, classes)
                    # Take absolute mean across samples and sum across classes
                    self.shap_values = np.abs(self.shap_values).mean(axis=0).sum(axis=1)
    ...
    def binomial_H0_test(array, n, p, alternative):
        ...
        Uses scipy.stats.binomtest for modern scipy versions or binom_test for older versions.
        """
        try:
            # Try modern scipy (>= 1.7.0)
            from scipy.stats import binomtest
            return [binomtest(int(x), n=n, p=p, alternative=alternative).pvalue for x in array]
        except ImportError:
            # Fall back to older scipy
